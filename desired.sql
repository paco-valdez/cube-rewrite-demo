...
FROM
  ...
  LEFT JOIN (select * from public.utilization_opportunity_materialized_value where product_id = 5)
   AS "estimated_opportunity" ON "estimated_opportunity".soc_id = "soc_zipcode".soc_id
WHERE
  (
